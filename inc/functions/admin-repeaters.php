...
function dm_sanitize_repeater($input)
{
    ...
    foreach ($input as $k => $v) {
        if (is_array($v)) {
            foreach ($v as $kk => $vv) {
                $input[$k][$kk] = sanitize_text_field($vv);
            }
        } else {
            ...
            switch ($tab) {
                ...
                case 'services':
                    dm_render_repeater_tab('dm_services', 'dm_services', array(
                        ...
                        array('name' => 'title', 'label' => 'Titre', 'type' => 'text'),
                        array('name' => 'desc', 'label' => 'Description', 'type' => 'textarea'),
                        array('name' => 'link', 'label' => 'Lien (URL relative)', 'type' => 'text'),
...
/* -------------------------------------------------------------------------- */
function dm_render_repeater_tab($option_name, $group, $fields, $max_items)
{
        ...
        <tr>
            ...
            <td>
                ...
                <p>
                    ...
                    <?php else : ?>
                        <input type="text" name="<?php echo esc_attr($option_name); ?>[<?php echo esc_attr($i); ?>][<?php echo esc_attr($field['name']); ?>]" value="<?php echo esc_attr($val); ?>" class="regular-text" />
                    <?php endif; ?>
                </p>
                <?php endforeach; ?>
...
function dm_render_stats_tab()
{
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                <input type="text" name="dm_stats_images[<?php echo esc_attr($i); ?>]" value="<?php echo esc_attr($img); ?>" placeholder="URL de l'image" class="regular-text" />
            </td>
        </tr>
        <?php endforeach; ?>
    </table>
